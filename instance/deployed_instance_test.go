...
var _ = Describe("DeployedInstance", func() {
	...
	Describe("PreRestoreLock", func() {
		...
		JustBeforeEach(func() {
			preRestoreLockError = deployedInstance.PreRestoreLock()
		})

		Context("when there are multiple pre-restore-lock scripts in multiple job directories", func() {
			BeforeEach(func() {
				jobs = orchestrator.Jobs([]orchestrator.Job{
					instance.NewJob(sshConnection, jobName+"/"+jobID, boshLogger, "", instance.BackupAndRestoreScripts{
						"/var/vcap/jobs/foo/bin/bbr/pre-restore-lock",
					}, instance.Metadata{}),
					instance.NewJob(sshConnection, jobName+"/"+jobID, boshLogger, "", instance.BackupAndRestoreScripts{
						"/var/vcap/jobs/bar/bin/bbr/pre-restore-lock",
					}, instance.Metadata{}),
					instance.NewJob(sshConnection, jobName+"/"+jobID, boshLogger, "", instance.BackupAndRestoreScripts{
						"/var/vcap/jobs/baz/bin/bbr/pre-restore-lock",
					}, instance.Metadata{}),
				})
			})
			...
			It("uses the ssh connection to run each post-restore-unlock script", func() {
				...
				Expect([]string{
					...
					"sudo /var/vcap/jobs/foo/bin/bbr/pre-restore-lock",
					"sudo /var/vcap/jobs/bar/bin/bbr/pre-restore-lock",
					"sudo /var/vcap/jobs/baz/bin/bbr/pre-restore-lock",
				))
			})

			It("logs the paths to the scripts being run", func() {
				Expect(string(stdout.Contents())).To(ContainSubstring(`> /var/vcap/jobs/foo/bin/bbr/pre-restore-lock`))
				Expect(string(stdout.Contents())).To(ContainSubstring(`> /var/vcap/jobs/bar/bin/bbr/pre-restore-lock`))
				Expect(string(stdout.Contents())).To(ContainSubstring(`> /var/vcap/jobs/baz/bin/bbr/pre-restore-lock`))
				Expect(string(stdout.Contents())).NotTo(ContainSubstring("> \n"))
			})
		...
		Context("when there are several scripts and one of them fails to run pre-restore-lock while another one causes an error", func() {
			...
			It("fails", func() {
				Expect(preRestoreLockError).To(HaveOccurred())
			})

			It("correctly reports a script failure", func() {
				By("describing the script failure in the returned error", func() {
					Expect(preRestoreLockError.Error()).To(ContainSubstring(
						fmt.Sprintf("lock script for job bar failed on %s/%s", jobName, jobID),
					))
				})

				By("including the script stdout in the returned error", func() {
					Expect(preRestoreLockError.Error()).To(ContainSubstring("Stdout: stdout_bar"))
				})

				By("including the script stderr in the returned error", func() {
					Expect(preRestoreLockError.Error()).To(ContainSubstring("Stderr: stderr_bar"))
				})
			...
			It("correctly reports failing to run a script (e.g. ssh connection failure)", func() {
				By("including the ssh error message in the returned error ", func() {
					Expect(preRestoreLockError.Error()).To(ContainSubstring("connection failed, script not run on baz"))
				})

				By("including the error message in the logs", func() {
					Expect(string(stderr.Contents())).To(ContainSubstring(fmt.Sprintf(
						"Error attempting to run pre-restore-lock script for job baz on %s/%s. Error: %s",
						jobName,
						jobID,
		...
		Context("When there are some jobs without pre-restore-lock scripts", func() {
			BeforeEach(func() {
				jobs = orchestrator.Jobs([]orchestrator.Job{
					instance.NewJob(sshConnection, jobName+"/"+jobID, boshLogger, "", instance.BackupAndRestoreScripts{
						"/var/vcap/jobs/job-has-lock-script/bin/bbr/pre-restore-lock",
					}, instance.Metadata{}),
					instance.NewJob(sshConnection, jobName+"/"+jobID, boshLogger, "", instance.BackupAndRestoreScripts{
			...
			It("Only invokes re-restore-lock on those jobs which have that script", func() {
				Expect(sshConnection.RunCallCount()).To(Equal(1))
				Expect(sshConnection.RunArgsForCall(0)).To(Equal("sudo /var/vcap/jobs/job-has-lock-script/bin/bbr/pre-restore-lock"))
			})
		})
	})
